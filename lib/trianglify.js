...
var _generate_points = points.generate_grid;
var _transform_points = points.transform_points;
...
function Trianglify(opts) {
  ...
  // setup seedable RNG
  var rand = seedrandom(opts.seed);

  // randomize colors if requested
  if (opts.x_colors === 'random') opts.x_colors = _random_from_palette(opts, rand);
  if (opts.y_colors === 'random') opts.y_colors = _random_from_palette(opts, rand);
  ...
  points = points ? _transform_points(points) : _generate_points(width, height, bleed_x, bleed_y, opts.cell_size, variance, rand);
  ...
  for (var i=0; i < geom_indices.length; i += 3) {
    var a = 2 * geom_indices[i];
    var b = 2 * geom_indices[i + 1];
    var c = 2 * geom_indices[i + 2];

    var ax = points[a], ay = points[a + 1];
    var bx = points[b], by = points[b + 1];
    var cx = points[c], cy = points[c + 1];

    var centroidX = (ax + bx + cx) / 3;
    var centroidY = (ay + by + cy) / 3;
    var color = gradient(norm_x(centroidX), norm_y(centroidY)).hex();
    triangles.push([color, [[ax, ay], [bx, by], [cx, cy]]]);
  }
  return new Pattern(triangles, opts);
...
function _random_from_palette(opts, rand) {
